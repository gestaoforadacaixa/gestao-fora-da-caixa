...
// ─── Bunker (ficha do cliente) ─────────────────────────────────────────────────
async function fetchClienteInfo(clienteId) {
  try {
    const r = await fetch(`${SUPA_URL}/rest/v1/clientes_info?cliente_id=eq.${clienteId}`, { headers: SBH });
    const arr = r.ok ? await r.json() : [];
    return arr[0] || null;
  } catch { return null; }
}
async function salvarClienteInfo(clienteId, dados) {
  try {
    const existe = await fetchClienteInfo(clienteId);
    const payload = { ...dados, cliente_id: clienteId, atualizado_em: new Date().toISOString() };
    if (existe) {
      const r = await fetch(`${SUPA_URL}/rest/v1/clientes_info?cliente_id=eq.${clienteId}`, { method: "PATCH", headers: { ...SBH_W, "Prefer": "return=minimal" }, body: JSON.stringify(payload) });
      return r.ok;
    } else {
      const r = await fetch(`${SUPA_URL}/rest/v1/clientes_info`, { method: "POST", headers: SBH_W, body: JSON.stringify(payload) });
      return r.ok;
    }
  } catch { return false; }
}
...
// ─── DETALHE ─────────────────────────────────────────────────────────────────────
// ─── BUNKER — ficha do cliente (Camada 1) ──────────────────────────────────────
function Bunker({ c }) {
  const CAMPOS = [
  ...
  const [dados, setDados] = useState({ razao_social: "", documento: "", responsavel: "", telefone: "", email: "", endereco: "", observacoes: "" });
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    let ativo = true;
    setLoading(true);
    fetchClienteInfo(c.id).then(info => {
      if (!ativo) return;
      if (info) setDados({ razao_social: info.razao_social || "", documento: info.documento || "", responsavel: info.responsavel || "", telefone: info.telefone || "", email: info.email || "", endereco: info.endereco || "", observacoes: info.observacoes || "" });
      setLoading(false);
    });
    return () => { ativo = false; };
  }, [c.id]);
  ...
  async function salvar() {
    setSalvando(true);
    const ok = await salvarClienteInfo(c.id, dados);
    setSalvando(false);
  ...
  return (
    <div className="fu">
      <div style={{ fontSize: 10, color: P.muted, letterSpacing: ".18em", textTransform: "uppercase", fontWeight: 600, marginBottom: 4 }}>Bunker</div>
      <div style={{ fontSize: 12, color: P.muted, marginBottom: 18 }}>Ficha de informações do cliente — visível só pra você.</div>
      <div className="glass" style={{ borderRadius: 16, padding: 18, boxShadow: P.shadow }}>
        {CAMPOS.map(([key, label]) => (
          <div key={key} style={{ marginBottom: 14 }}>
            <label style={{ fontSize: 9.5, color: P.muted, letterSpacing: ".1em", textTransform: "uppercase", fontWeight: 600, display: "block", marginBottom: 6 }}>{label}</label>
            <input className="inp" value={dados[key]} onChange={e => setDados(d => ({ ...d, [key]: e.target.value }))} placeholder={label} />
          </div>
        ))}
        <div style={{ marginBottom: 4 }}>
          <label style={{ fontSize: 9.5, color: P.muted, letterSpacing: ".1em", textTransform: "uppercase", fontWeight: 600, display: "block", marginBottom: 6 }}>Observações</label>
          <textarea className="inp" style={{ minHeight: 80, resize: "none", fontFamily: "'Sora',sans-serif" }} value={dados.observacoes} onChange={e => setDados(d => ({ ...d, observacoes: e.target.value }))} placeholder="Anotações gerais, contexto, lembretes…" />
        </div>
      </div>
      <button className="btn btn-main" style={{ background: c.cor, color: "#1A1D24", marginTop: 14 }} onClick={salvar} disabled={salvando}>
        {salvando ? <><span className="spin" /> Salvando</> : salvo ? "✓ Salvo" : "Salvar Ficha"}
      </button>
    </div>
  );
}
          ...
          <div className="sheet">
            ...
            {checandoUso ? (
              ...
              <>
                ...
                <button className="btn" style={{ background: P.red, color: "#fff" }} onClick={confirmarExclusao} disabled={excluindo}>{excluindo ? <><span className="spin" /> Excluindo</> : "Confirmar Exclusão"}</button>
              </>
            ) : (
              <>
                ...
                <button className="btn" style={{ background: P.red, color: "#fff" }} onClick={confirmarExclusao} disabled={excluindo || !migrarPara}>
    ...
    <div>
      ...
      <div className="glass" style={{ borderRadius: 0, borderLeft: "none", borderRight: "none", borderTop: "none", padding: "14px 20px 0", position: "sticky", top: 0, zIndex: 10, boxShadow: "0 4px 18px rgba(60,100,150,.07)" }}>
        ...
        <div style={{ display: "flex", borderTop: `1px solid ${P.border}` }}>
          {[["caixa", "Caixa"], ["dashboard", "Dashboard"], ["relatorio", "Relatório"], ["historico", "Histórico"], ["categorias", "Categorias"], ["bunker", "Bunker"]].map(([v, l]) => (
